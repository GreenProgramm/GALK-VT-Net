...
class Res_lsknet_attn_text_visual_fgtpg(nn.Module):
    def __init__(self, img_size, in_channels=1,out_channels=13, text_embeddings_path = None,Training = True ):
        ...
        # self.vtf=Ca_atten_dwconv(dim=1,num_heads=1)
    ...
    def forward(self, x_in):
        ...
        out = self.conv_prompt(prompt) + out_backbone
        
#         out_list = []
#         for i in range(self.out_channels):                                                  
            
#             out_backbone_ = out_backbone[:,i,:].unsqueeze(1)
#             prompt_ = prompt[:,i,:].unsqueeze(1)
...
#             visual_text = self.vtf(out_backbone_,prompt_,prompt_)
