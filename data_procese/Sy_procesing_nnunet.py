...
file_list_train = open(os.path.join(main_path, 'Sy_train.txt')).readlines()
file_list_test = open(os.path.join(main_path, 'Sy_test.txt')).readlines()
input_path = main_path + '/Training/img/'
for i in range(2):
    if i==0:
        file_list = file_list_train
        save_file = 'Training_3D_nnunet'
    elif i==1:
        file_list = file_list_test
        save_file = 'Testing_3D_nnunet'
