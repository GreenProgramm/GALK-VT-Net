...
def resampleVolume(data_dict):
    ...
    data_image, data_label = data_dict["image"].squeeze().permute(1, 0, 2), data_dict["label"].squeeze().permute(1, 0,
                                                                                                                 2)
...
main_path = r'D:\Project\multi-organ seg\btcv'
file_list_train = open(os.path.join(main_path, 'BTCV_train.txt')).readlines()
file_list_val = open(os.path.join(main_path, 'BTCV_val.txt')).readlines()
file_list_test = open(os.path.join(main_path, 'BTCV_val.txt')).readlines()
...
for i in range(3):
    ...
    for patient_name in file_list:
        ...
        data_masks_str = input_path.replace('images', 'labels') + 'label' + patient_name[3:] + '.nii.gz'
